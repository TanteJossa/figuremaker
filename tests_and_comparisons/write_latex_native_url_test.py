...
def main():
    # Known native URL from a previously successful in-Slides copy.
    # The file is already hosted on Drive and the native copy format uses
    # exactly this URL, so if this test fails the issue is payload structure
    # rather than the uploaded file/permissions.
    ...
    print("Clipboard MIME pairs:")
    for k, v in pairs:
        print(f"  {k}: {len(v)} chars")

    if set_clipboard_data(pairs, text_fallback="LaTeX native URL test"):
        print("\nClipboard set with a known native Drive URL. Paste into Google Slides now (Ctrl+V).")
        print("If this still shows a retrieval error, the problem is the payload structure/clipboard wrapper.")
        print("If this pastes correctly, the problem is the Drive upload/URL returned by the site.")
    else:
