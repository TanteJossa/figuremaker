...
def wrap_flat(flat_payload, dih=1245482604,
    ...
    wrapped = {
        "dih": dih,
        "data": json.dumps(flat_payload, separators=(',', ':')),
        "edi": edi,
        "edrk": edrk,
        "dct": "punch",
        "ds": False,
...
def main():
    src_path = os.path.join("clipboard_iterations", "test_latex_view", "copied_structure.json")
    with open(src_path, "r", encoding="utf-8") as f:
        native_flat = json.load(f)

    print(f"Loaded native flat payload: {len(native_flat.get('resolved', []))} resolved ops")

    clip_id = f"2519a9aa-7fff-ab00-1255-{uuid.uuid4().hex[:12]}"

    pairs = [
        ("application/x-vnd.google-docs-internal-clip-id", clip_id),
        ("application/x-vnd.google-docs-drawings-object+wrapped", wrap_flat(native_flat)),
    ]
    ...
    if set_clipboard_data(pairs, text_fallback="Native LaTeX payload test"):
        print("\nClipboard set with the exact native flat payload. Paste into Google Slides now (Ctrl+V).")
        print("If this pastes correctly, our SlidesBuilder is generating an invalid image element.")
        print("If this still fails, the wrapper signatures or missing auxiliary MIME types are the cause.")
    else:
