...
def main():
    ...
    # Build with the real Drive URL (so the placeholder references a valid file).
    ...
    # Override the image-clip wrapper to use the inline data URL instead.
    image_clip_data = json.loads(payload["image_clip_wrapped"])
    image_clip_inner = json.loads(image_clip_data["data"])
    blob_id = list(image_clip_inner["image_urls"].keys())[0]
    image_clip_inner["image_urls"][blob_id] = data_url
    image_clip_data["data"] = json.dumps(image_clip_inner, separators=(',', ':'))
    image_clip_wrapped = json.dumps(image_clip_data, separators=(',', ':'))
    ...
    pairs = [
        ...
        ("application/x-vnd.google-docs-image-clip+wrapped", image_clip_wrapped),
    ]

    print("\nClipboard MIME pairs (Drive URL in shape, data URL in image-clip):")
    for k, v in pairs:
        print(f"  {k}: {len(v)} chars")

    if set_clipboard_data(pairs, png_bytes=png_bytes, text_fallback="LaTeX hybrid URL test"):
