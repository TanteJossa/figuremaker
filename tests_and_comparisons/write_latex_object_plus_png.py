...
def main():
    ...
    # Build drawings-object wrapper with a dummy/empty image URL.
    builder = SlidesBuilder(font_family='Ubuntu')
    builder.add_image(
        x=272.4, y=79.5,
        width_pt=latex_meta['width'], height_pt=latex_meta['height'],
        image_url='',  # empty URL; we hope Google Slides falls back to PNG bytes
    ...
    # Only drawings-object + PNG bytes (no document-slice/image-clip)
    pairs = [
