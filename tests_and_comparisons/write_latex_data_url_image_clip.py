...
def main():
    ...
    print("\nClipboard MIME pairs (data URL in image-clip):")
    for k, v in pairs:
        print(f"  {k}: {len(v)} chars")

    if set_clipboard_data(pairs, png_bytes=png_bytes, text_fallback="LaTeX data URL test"):
        print("\nClipboard set with data URL in image-clip + PNG bytes. Paste into Google Slides now (Ctrl+V).")
    else:
