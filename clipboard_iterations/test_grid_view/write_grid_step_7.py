...
def main():
    print("=========================================================")
    print("   WRITING GRID STEP 7: FINAL COMPLETED METADATA GRIDS   ")
    print("=========================================================")
    ...
    # 7. Blue curves (color: #0B5394)
    blue_lines = [op for op in unresolved if len(op) > 1 and op[0] == 3 and op[2] == 153 and len(op) > 4 and op[4][7] == "#0B5394"]
    
    # 8. Red curves (color: #980000)
    red_lines = [op for op in unresolved if len(op) > 1 and op[0] == 3 and op[2] == 153 and len(op) > 4 and op[4][7] == "#980000"]
    
    # 9. Group Operations (opcode: 2)
    group_ids = [
        "slide_0b5480b5453b_g_mg_c_9z77", 
        "slide_0b5480b5453b_g_mg_c_gkth", 
        "slide_0b5480b5453b_g_mg_c_66rs", 
        "slide_0b5480b5453b_g_mg_c_uqmt", 
        "slide_0b5480b5453b_g_mg_c_kwsj", 
        "slide_0b5480b5453b_g_mg_c_4btw"
    ]
    group_ops = [op for op in unresolved if len(op) > 1 and op[1] in group_ids]
    ...
    print(f"[+] Loaded {len(blue_lines)} blue series curve segments.")
    print(f"[+] Loaded {len(red_lines)} red series curve segments.")
    print(f"[+] Loaded {len(group_ops)} curve group operations.")
    
    # Combine everything to exactly reproduce the original full figure GWT structure
    resolved_array = title_ops + rect_ops + grey_lines + tick_ops + header_ops + green_lines + blue_lines + red_lines + group_ops
    ...
    success = set_clipboard_data(pairs, text_fallback="[Google Slides Kinematics Grid Step 7]")
    if success:
        print("\n[+] Success! Grid Step 7 (FINAL COMPLETED METADATA GRIDS) has been written.")
        print("    You can now paste (Ctrl+V) directly onto any active Google Slides slide.")
    else:
        print("\n[!] Error: Failed to write final Step 7 payload.")
