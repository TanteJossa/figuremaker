...
def main():
    print("=========================================================")
    print("   WRITING GRID STEP 3: STEP 2 + 84 TICK MARKS/LABELS    ")
    print("=========================================================")
    ...
    # 4. Axis Tick Marks and Variable Labels (0, 1, 2, 3, 4, 5, t, x, v)
    tick_shape_ids = set()
    ...
    # Combine together
    resolved_array = title_ops + rect_ops + grey_lines + tick_ops
    ...
    success = set_clipboard_data(pairs, text_fallback="[Google Slides Kinematics Grid Step 3]")
    if success:
        print("\n[+] Success! Grid Step 3 (Title + Panels + Gridlines + Ticks/Labels) has been written.")
        print("    You can now paste (Ctrl+V) directly onto any active Google Slides slide.")
    else:
        print("\n[!] Error: Failed to write Step 3 payload.")
