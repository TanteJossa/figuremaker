...
def encode_chromium_web_custom(pairs):
    data = b""
    for k, v in pairs:
        # Key length and UTF-16LE bytes
        data += len(k).to_bytes(4, "little") + k.encode("utf-16le")
        if len(k) % 2 != 0:
            data += b"\0\0"
        # Value length and UTF-16LE bytes
        data += len(v).to_bytes(4, "little") + v.encode("utf-16le")
        if len(v) % 2 != 0:
            data += b"\0\0"
    
    # Total length of data (pairs count + pairs data)
...
def set_clipboard_data(pairs, text_fallback=""):
    ...
    try:
        ...
        # 1. Write Chromium Custom format
        ...
        if h_mem:
            ...
            if p_mem:
                ...
                print("Successfully wrote Chromium Web Custom MIME Data Format to system clipboard.")
                
        # 2. Write Unicode Text fallback
        if text_fallback:
            ...
            if h_text:
                ...
                if p_text:
                    ...
                    print("Successfully wrote fallback UnicodeText.")
...
if __name__ == "__main__":
    # Define custom drawings object JSON structure
    resolved_array = [
        # Action, objectId, ShapeType(6=rect), [scaleX, 0, 0, scaleY, tx, ty], [15(fill), fillColor, 19(stroke), strokeColor, 22(weight), strokeWidth], Parent
    ...
    set_clipboard_data(pairs, text_fallback="[Google Slides Rect]")
    print("\nTest rectangle set! Please go to Google Slides and press Ctrl+V. Let us know if a light blue rectangle pastes successfully!")
